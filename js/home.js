...
auth.onAuthStateChanged(user => {
  if (user) {
    ...
    const safeActivityUrl = url => {
      ...
      return /^(notifications|chat|group|event)\.html(?:[?#]|$)/.test(value)
    ...
    // ✅ Scoped listeners to current user only
    onValue(ref(db, `users/${user.uid}/profile`), snapshot => {
      profiles = {};
      if (snapshot.exists()) profiles[user.uid] = snapshot.val();
      renderActivity();
    });
...
["chats", "groups", "events"].forEach(source => 
  onValue(ref(db, `users/${user.uid}/${source}`), snapshot => {
    ...
    // Notifications listener (already scoped correctly)
